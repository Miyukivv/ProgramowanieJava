...
//        return  people.stream().sorted(Comparator.comparing(person -> person.getBirthDate())).collect(Collectors.toList());
//    }
//
//    //zad 6
//    public static List<Person> getDeadPeople(List<Person> people){
...
//     */
//
//    public static Person theOldestPerson(List<Person> people){
//
//        //Moje:
//       // List <Person> tmp = people.stream().filter(p -> p.deathDate == null).sorted(Comparator.comparing(person -> person.getLifeLength())).collect(Collectors.toList());
//        //return tmp.get(tmp.size()-1);
//
//        return people.stream().filter(person -> person.deathDate == null).min(Comparator.comparing(person -> person.getBirthDate())).orElse(null);
//    }
